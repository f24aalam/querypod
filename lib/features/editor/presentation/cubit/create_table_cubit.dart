...
import 'connection_metadata_cubit.dart';

class CreateTableCubit extends Cubit<CreateTableState> {
  final ConnectionCubit _connectionCubit;
  final ConnectionMetadataCubit _workspaceMetadataCubit;

  CreateTableCubit({
    required ConnectionCubit connectionCubit,
    required ConnectionMetadataCubit workspaceMetadataCubit,
  ...
  void addColumn() {
    ...
    columns.add(
      TableColumnDefinition(name: '', type: defaultType, length: defaultLength),
    );
  ...
  Future<void> submit(String database, String? schema) async {
    ...
    if (state.columns.isEmpty) {
      emit(
        state.copyWith(errorMessage: 'Table must have at least one column.'),
      );
      return;
    }
    ...
    try {
      if (state.originalTableName != null && state.originalColumns != null) {
        ...
        await _workspaceMetadataCubit.createTable(
          connection,
          database,
          schema,
          state.tableName,
  ...
  Future<void> loadTableSchema(
    ...
    try {
      final columns = await _workspaceMetadataCubit.getTableSchema(
        connection,
        database,
        schema,
        table,
